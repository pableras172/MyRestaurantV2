...
{
	// Si el body tiene un data-color definido por el servidor (restaurante), usar ese en lugar de cookies
	var serverThemeColor = jQuery('body').attr('data-color');
	if (serverThemeColor && serverThemeColor.indexOf('color_') > -1) {
		// El color viene del servidor, no usar cookies para el color
		themeOptionArr['themeFullColor_value'] = serverThemeColor;
		// Mantener el resto de opciones desde cookies o defaults
		jQuery.each(themeOptionArr, function(optionKey, optionValue) {
			if (optionKey !== 'themeFullColor_value') {
				var optionData = getCookie(optionKey);
				themeOptionArr[optionKey] = (optionData != '')?optionData:dzSettingsOptions[optionKey];
			}
		});
	} else {
		// No hay color del servidor, usar cookies normalmente
		jQuery.each(themeOptionArr, function(optionKey, optionValue) {
			var optionData = getCookie(optionKey);
			themeOptionArr[optionKey] = (optionData != '')?optionData:dzSettingsOptions[optionKey];
		});
...
function setThemePanel(){
	// Obtener el color del servidor si existe
	var serverThemeColor = jQuery('body').attr('data-color');
	
	jQuery.each(themeOption, function(index, themeOptionItem) {
		
		// Si es themeFullColor y hay un color del servidor, usar ese en lugar de la cookie
		if (themeOptionItem == 'themeFullColor' && serverThemeColor && serverThemeColor.indexOf('color_') > -1) {
			themeOptionItemValue = serverThemeColor;
		} else {
