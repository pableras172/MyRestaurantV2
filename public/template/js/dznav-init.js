...
(function($) {
	...
	// Detectar si hay un color definido por el servidor (restaurante)
	var serverThemeColor = $('body').attr('data-color');
	var themeColor = "color_1"; // default
	
	// Si el body tiene un data-color del servidor, usarlo
	if (serverThemeColor && serverThemeColor.indexOf('color_') > -1) {
		themeColor = serverThemeColor;
	}
	
	dzSettingsOptions = {
		themeFullColor_value: themeColor,
